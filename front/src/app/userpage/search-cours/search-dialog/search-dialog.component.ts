...
export class SearchDialogComponent implements OnInit, AfterViewInit {
  ...
  searchDialogForm: FormGroup;
    ...
    displayedColumns: string[] = ['action', 'titre' , 'horaire', 'taille_groupe', 'niveau', 'recurrent', 'moniteur'];
        ...
        if (this.selectedMoniteur > 0 && !(this.selectedMoniteur === undefined)) {
            ...
            if (this.selectedNiveau > 0 && !(this.selectedNiveau === undefined) ){
              ...
              this.coursService.getAllCours() // changer l'id ICI
                  .subscribe((data: any[]) => this.chargeData(data.filter(item => item.niveau === this.selectedNiveau && item.idMoniteur === this.selectedMoniteur )));
            }
            else {
              this.coursService.getAllCours() // changer l'id ICI
                  .subscribe((data: any[]) => this.chargeData(data.filter(item => item.idMoniteur === this.selectedMoniteur )));
          ...
          if (this.selectedNiveau > 0 && !(this.selectedNiveau === undefined)){
              ...
              this.coursService.getAllCours() // changer l'id ICI
                  .subscribe((data: any[]) => this.chargeData(data.filter(item => item.niveau === this.selectedNiveau)));
            }
            else {
              this.coursService.getAllCours() // changer l'id ICI
                  .subscribe((data: any[]) => this.chargeData(data));
            }
        }
    ...
    // MET a jour les liste du composant
     ...
     public isCoursSelected(id_cours){
      return this.current_cours.some(item =>  item.idCours === id_cours);
     }
